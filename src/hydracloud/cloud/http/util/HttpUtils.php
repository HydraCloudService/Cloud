...
use hydracloud\cloud\http\io\data\Queries;
use hydracloud\cloud\http\io\data\RequestData;
...
final class HttpUtils {
	...
	public static function parseRequest(Address $address, string $request): ?Request {
		...
		if (filter_var(self::LOCALHOST_PREFIX . $path, FILTER_VALIDATE_URL) !== self::LOCALHOST_PREFIX . $path) {
            return null;
        }
        ...
        if (str_contains($path, "?")) {
            $queries = substr($path, strpos($path, "?") + 1);
        }
		...
		return new Request(self::decodeHeaders($headers), new RequestData($address, strtoupper($method), urldecode($path), new Queries(self::remapQueries($queries))), implode(PHP_EOL, array_filter($bodyLines, static fn($v) => $v)));
	}
	...
	private static function remapQueries(string $queries): array {
		...
		foreach (explode("&", $queries) as $pair) {
			if (substr_count($pair, "=") !== 1 || str_starts_with($pair, "=") || str_ends_with($pair, "=")) {
                continue;
            }
    ...
    public static function matchPath(string $base, string $match): bool {
        if (str_contains($base, "?")) {
            ...
            if (!self::matchQueries(substr($base, strpos($base, "?") + 1), substr($match, strpos($match, "?") + 1))) {
                return false;
            }
        ...
        if (count($baseParameters) !== count($matchParameters)) {
            return false;
        }
        ...
        foreach ($baseParameters as $k => $v) {
            ...
            if (str_starts_with($v, "{") && str_ends_with($v, "}") && !Pattern::isValid($matchParameters[$k], json_decode($v, true, 512, JSON_THROW_ON_ERROR))) {
                return false;
            }

            if ($v !== $matchParameters[$k]) {
                return false;
            }
        }
        return true;
    ...
    public static function fillRequest(Request $request, string $baseUrl): void {
		...
		foreach ($baseParameters as $k => $v) {
			if (str_starts_with($v, "#")) {
				...
				$d = json_decode($v, true, 512, JSON_THROW_ON_ERROR);

				if (!isset($d["name"])) {
                    throw new UnexpectedValueException("Parameter name for $baseUrl (" . $request->data()->method() . ") is missing!");
                }
