...
namespace hydracloud\cloud\exception;
...
final class ExceptionHandler {
    ...
    public static function handle(Throwable $throwable): void {
        $send = true;
        if (($throwable instanceof ErrorException) && (error_reporting() & $throwable->getSeverity()) === 0) {
            $send = false;
        }
    ...
    public static function set(): void {
        set_error_handler(static fn(int $errno, string $error, string $file, int $line) => self::handle(new ErrorException($error, 0, $errno, $file, $line)));
