...
final class CrashChecker {
    ...
    public static function checkCrashed(CloudServer $server, ?array &$crashData = null): bool {
        ...
        if (!file_exists($server->getPath() . "crashdumps/")) {
            return false;
        }

        foreach (array_diff(scandir($server->getPath() . "crashdumps/"), [".", ".."]) as $file) {
            if ((pathinfo($server->getPath() . "crashdumps/" . $file, PATHINFO_EXTENSION) === "log") && (time() - filectime($server->getPath() . "crashdumps/" . $file)) <= 60) {
    ...
    public static function writeCrashFile(CloudServer $server, array $crashData): void {
        ...
        $data = [
            ...
            "PHP Version" => PHP_VERSION .((function_exists('opcache_get_status') && ($opcacheStatus = opcache_get_status(false)) !== false && ($opcacheStatus["jit"]["on"] ?? false)) ? " (JIT enabled)" : " (JIT disabled)")
        ];
