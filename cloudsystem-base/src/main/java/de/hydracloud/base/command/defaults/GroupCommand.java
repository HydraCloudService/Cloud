...
import de.hydracloud.base.group.DefaultGroup;
...
public final class GroupCommand extends CloudCommand {
            ...
            groupManager.getServiceGroupByName(args[0]).ifPresentOrElse(group -> {
                ...
                switch (key) {
                    ...
                    case "startnewpercentage" -> {
                        this.getAndSetDouble(key, args[3], group, value -> {
                            group.setStartNewPercentage(value);
                            base.getDatabaseManager().getProvider()
                                .updateGroupProperty(group.getName(), "startNewPercentage", value);
                        });
                        logger.log("§7Successfully set start percentage to " + args[3]);
                    }
                    case "fallback" -> {
        ...
        if (arguments.length == 1) {
            ...
            if (arguments[1].equalsIgnoreCase("edit")) {
                return Arrays.asList("memory", "minServiceCount", "maxServiceCount",
                    "defaultMaxPlayers", "startnewpercentage", "fallback", "maintenance", "version");
            }
    ...
    private void getAndSetDouble(final String key, final String value, final ServiceGroup group, final Consumer<Double> consumer) {
        try {
            consumer.accept(Double.parseDouble(value));
            group.update();
        } catch (NumberFormatException e) {
            CloudAPI.getInstance().getLogger()
                .log("§7Use following command: §bgroup edit " + group.getName() + " " + key + " §7(§bdouble§7)");
        }
    }
}
